...
static rt_err_t _rndis_query_response(ufunction_t func,rndis_query_msg_t msg)
{
...
#ifdef  RNDIS_DELAY_LINK_UP
        if(((rt_rndis_eth_t)func->user_data)->link_status)
        {
    ...
    {
        const char vendor_desc[] = "TXW_RNDIS";
...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size)
{
    ...
    if(((rt_rndis_eth_t)func->user_data)->rx_offset >= ((rt_rndis_eth_t)func->user_data)->rx_length)
    {
        ...
        if (((rt_rndis_eth_t)func->user_data)->input_cb) {
            ((rt_rndis_eth_t)func->user_data)->input_cb(
                &((rt_rndis_eth_t)func->user_data)->parent,
                (rt_uint8_t *)((rt_rndis_eth_t)func->user_data)->rx_buffer,
                ((rt_rndis_eth_t)func->user_data)->rx_length,
                ((rt_rndis_eth_t)func->user_data)->input_priv);
        }
    }
    // input_cb同步处理，每次都可以重新发送usb请求以便下次接收
...
static rt_err_t _function_enable(ufunction_t func)
{
    ...
    ((rt_rndis_eth_t)func->user_data)->rx_frist = RT_TRUE;
    // eth_device_ready(&(((rt_rndis_eth_t)func->user_data)->parent));
    ((rt_rndis_eth_t)func->user_data)->link_status = TRUE;
...
static rt_err_t _function_disable(ufunction_t func)
{
    ...
    /* link down. */
    //eth_device_linkchange(&((rt_rndis_eth_t)func->user_data)->parent, RT_FALSE);
    ((rt_rndis_eth_t)func->user_data)->link_status = FALSE;
...
#else
static int32 rndis_dev_open(struct netdev *ndev, netdev_input_cb cb, netdev_event_cb evt_cb, void *priv)
{
    rt_rndis_eth_t device = rt_container_of(ndev, struct rt_rndis_eth, parent);
    
    device->input_cb   = cb;
    device->event_cb   = evt_cb;
    device->input_priv = priv;
    //device->link_status = TRUE;

    return 0;
}

static int32 rndis_dev_close(struct netdev *ndev)
{
    rt_rndis_eth_t device = rt_container_of(ndev, struct rt_rndis_eth, parent);

    device->input_cb   = NULL;
    device->event_cb   = NULL;
    device->input_priv = NULL;
    //device->link_status = FALSE;

    return 0;
}

static int32 rndis_dev_send(struct netdev *ndev, uint8 *data, uint32 size)
{
    char * buffer;
    rt_err_t result = RT_EOK;
    rt_rndis_eth_t device = rt_container_of(ndev, struct rt_rndis_eth, parent);

    if(!device->link_status)
    {
        LOG_I("linkdown, drop pkg");
        return RT_EOK;
    }
    ...
    //RT_ASSERT(p->tot_len < sizeof(device->tx_buffer));
    if(size > sizeof(device->tx_buffer))
    {
        LOG_W("RNDIS MTU is:%d, but the send packet size is %d",
                     sizeof(device->tx_buffer), size);
        size = sizeof(device->tx_buffer);
    }
    ...
    /* send */
    {
        ...
        msg->VcHandle = 0;
        msg->Reserved = 0;
        msg->MessageLength = sizeof(struct rndis_packet_msg) + size;
...
static int32 rndis_dev_scatter_send(struct netdev *ndev, scatter_data *data, uint32 count)
{
    char * buffer;
    rt_err_t result = RT_EOK;
    rt_rndis_eth_t device = rt_container_of(ndev, struct rt_rndis_eth, parent);
    ...
    // 拷贝scatter中数据
    for (i = 0; i < count; ++i) {
        rt_memcpy(buffer + offset, data[i].addr, data[i].size);
        offset += data[i].size;
    }
...
static int32 rndis_dev_ioctl(struct netdev *ndev, uint32 cmd, uint32 param1, uint32 param2)
{
    return 0;
}

static const struct netdev_hal_ops rndis_device_ops = {
    .open      = rndis_dev_open,
    .close     = rndis_dev_close,
    .ioctl     = rndis_dev_ioctl,
    .send_data = rndis_dev_send,
    .send_scatter_data = rndis_dev_scatter_send,
};
#endif
...
ufunction_t rt_usbd_function_rndis_create(udevice_t device)
{
    ...
    sysctrl_efuse_mac_addr_calc(_rndis->host_addr);
    _rndis->host_addr[5]++;
...
#else
    _rndis->parent.dev.ops              = (const struct devobj_ops *)&rndis_device_ops;
    dev_register(HG_LTE_RNDIS_DEVID, &_rndis->parent.dev);
#endif
