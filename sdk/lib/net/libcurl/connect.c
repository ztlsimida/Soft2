...
struct eyeballer {
  ...
  CURLcode result;
  int error;
  _BIT_(rewinded);                     /* if we rewinded the addr list */
  _BIT_(has_started);                  /* attempts have started */
  _BIT_(is_done);                      /* out of addresses/time */
  _BIT_(connected);                    /* cf has connected */
  _BIT_(shutdown);                     /* cf has shutdown */
  _BIT_(inconclusive);                 /* connect was not a hard failure, we
