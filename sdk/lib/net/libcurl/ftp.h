...
struct ftp_conn {
  ...
  _BIT_(ftp_trying_alternative);
  _BIT_(dont_check);  /* Set to TRUE to prevent the final (post-transfer)
                       file size and 226/250 status check. It should still
                       read the line, just ignore the result. */
  _BIT_(ctl_valid);   /* Tells Curl_ftp_quit() whether or not to do anything. If
                       the connection has timed out or been closed, this
                       should be FALSE when it gets to Curl_ftp_quit() */
  _BIT_(cwddone);     /* if it has been determined that the proper CWD combo
                       already has been done */
  _BIT_(cwdfail);     /* set TRUE if a CWD command fails, as then we must prevent
                       caching the current directory */
  _BIT_(wait_data_conn); /* this is set TRUE if data connection is waited */
};
