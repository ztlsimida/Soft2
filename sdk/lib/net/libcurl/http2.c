...
struct cf_h2_ctx {
  ...
  _BIT_(initialized);
  _BIT_(via_h1_upgrade);
  _BIT_(conn_closed);
  _BIT_(rcvd_goaway);
  _BIT_(sent_goaway);
  _BIT_(enable_push);
  _BIT_(nw_out_blocked);
};
...
struct h2_stream_ctx {
  ...
  _BIT_(resp_hds_complete); /* we have a complete, final response */
  _BIT_(closed); /* TRUE on stream close */
  _BIT_(reset);  /* TRUE on stream reset */
  _BIT_(close_handled); /* TRUE if stream closure is handled by libcurl */
  _BIT_(bodystarted);
  _BIT_(body_eos);    /* the complete body has been added to `sendbuf` and
