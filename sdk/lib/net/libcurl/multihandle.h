...
#define IPV6_WORKS   2
  ...
  _BIT_(multiplexing);           /* multiplexing wanted */
  _BIT_(recheckstate);           /* see Curl_multi_connchanged */
  _BIT_(in_callback);            /* true while executing a callback */
#ifdef USE_OPENSSL
  _BIT_(ssl_seeded);
#endif
  _BIT_(dead); /* a callback returned error, everything needs to crash and
                burn */
  _BIT_(xfer_buf_borrowed);      /* xfer_buf is currently being borrowed */
  _BIT_(xfer_ulbuf_borrowed);    /* xfer_ulbuf is currently being borrowed */
  _BIT_(xfer_sockbuf_borrowed);  /* xfer_sockbuf is currently being borrowed */
#ifdef DEBUGBUILD
  _BIT_(warned);                 /* true after user warned of DEBUGBUILD */
#endif
};
