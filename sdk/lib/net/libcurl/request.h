...
#endif
  _BIT_(header);        /* incoming data has HTTP header */
  _BIT_(done);          /* request is done, e.g. no more send/recv should
                       * happen. This can be TRUE before `upload_done` or
                       * `download_done` is TRUE. */
  _BIT_(content_range); /* set TRUE if Content-Range: was found */
  _BIT_(download_done); /* set to TRUE when download is complete */
  _BIT_(eos_written);   /* iff EOS has been written to client */
  _BIT_(eos_read);      /* iff EOS has been read from the client */
  _BIT_(eos_sent);      /* iff EOS has been sent to the server */
  _BIT_(rewind_read);   /* iff reader needs rewind at next start */
  _BIT_(upload_done);   /* set to TRUE when all request data has been sent */
  _BIT_(upload_aborted); /* set to TRUE when upload was aborted. Will also
                        * show `upload_done` as TRUE. */
  _BIT_(ignorebody);    /* we read a response-body but we ignore it! */
  _BIT_(http_bodyless); /* HTTP response status code is between 100 and 199,
                         204 or 304 */
  _BIT_(chunk);         /* if set, this is a chunked transfer-encoding */
  _BIT_(resp_trailer);  /* response carried 'Trailer:' header field */
  _BIT_(ignore_cl);     /* ignore content-length */
  _BIT_(upload_chunky); /* set TRUE if we are doing chunked transfer-encoding
                         on upload */
  _BIT_(getheader);    /* TRUE if header parsing is wanted */
  _BIT_(no_body);      /* the response has no body */
  _BIT_(authneg);      /* TRUE when the auth phase has started, which means
                        that we are creating a request with an auth header,
                        but it is not the final request in the auth
                        negotiation. */
  _BIT_(sendbuf_init); /* sendbuf is initialized */
  _BIT_(shutdown);     /* request end will shutdown connection */
  _BIT_(shutdown_err_ignore); /* errors in shutdown will not fail request */
#ifdef USE_HYPER
  _BIT_(bodywritten);
#endif
};
