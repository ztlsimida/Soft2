...
#endif
  _BIT_(noauthpersist);
  _BIT_(havenoauthpersist);
  _BIT_(havenegdata);
  _BIT_(havemultiplerequests);
};
#endif
...
struct ConnectBits {
  ...
  _BIT_(bound); /* set true if bind() has already been done on this socket/
                 connection */
  _BIT_(asks_multiplex); /* connection asks for multiplexing, but is not yet */
...
struct Curl_async {
  ...
  _BIT_(done);  /* set TRUE when the lookup is complete */
};
...
#ifdef HAVE_GSSAPI
  _BIT_(sec_complete); /* if Kerberos is enabled for this connection */
...
#if defined(HAVE_GSSAPI)
  _BIT_(krb);            /* Kerberos connection requested */
#endif
