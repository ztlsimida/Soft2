...
struct cf_osslq_ctx {
  ...
  _BIT_(initialized);
  _BIT_(got_first_byte);               /* if first byte was received */
  _BIT_(x509_store_setup);             /* if x509 store has been set up */
  _BIT_(protocol_shutdown);            /* QUIC connection is shut down */
  _BIT_(need_recv);                    /* QUIC connection needs to receive */
  _BIT_(need_send);                    /* QUIC connection needs to send */
};
