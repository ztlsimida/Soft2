...
struct cf_quiche_ctx {
  ...
  _BIT_(initialized);
  _BIT_(goaway);                       /* got GOAWAY from server */
  _BIT_(x509_store_setup);             /* if x509 store has been set up */
  _BIT_(shutdown_started);             /* queued shutdown packets */
};
...
struct stream_ctx {
  ...
  _BIT_(opened); /* TRUE after stream has been opened */
  _BIT_(closed); /* TRUE on stream close */
  _BIT_(reset);  /* TRUE on stream reset */
  _BIT_(send_closed); /* stream is locally closed */
  _BIT_(resp_hds_complete);  /* final response has been received */
  _BIT_(resp_got_header); /* TRUE when h3 stream has recvd some HEADER */
  _BIT_(quic_flow_blocked); /* stream is blocked by QUIC flow control */
};
