...
BinaryDelayEstimatorFarend *WebRtc_CreateBinaryDelayEstimatorFarend(
    ...
    if (history_size > 1) {
        ...
        self = (BinaryDelayEstimatorFarend *)(
                webrtc_malloc(sizeof(BinaryDelayEstimatorFarend)));
    }
    if (self == NULL) {
        return NULL;
    }
...
int WebRtc_AllocateFarendBufferMemory(BinaryDelayEstimatorFarend *self,
    ...
    self->far_bit_counts = (int *)(realloc(
...
BinaryDelayEstimator *WebRtc_CreateBinaryDelayEstimator(
    ...
    if ((farend != NULL) && (max_lookahead >= 0)) {
        ...
        self = (BinaryDelayEstimator *)(
                webrtc_malloc(sizeof(BinaryDelayEstimator)));
    }
    if (self == NULL) {
        return NULL;
    }
    ...
    self->binary_near_history = (uint32_t *)(
            webrtc_malloc((max_lookahead + 1) * sizeof(*self->binary_near_history)));
...
//模拟c++的std::any_of
#if 1
int any_of(int arr[], int size, int (*predicate)(int)) {
...
int WebRtc_ProcessBinarySpectrum(BinaryDelayEstimator *self,
    ...
    const bool non_stationary_farend =
            std::any_of(self->farend->far_bit_counts,
                        ...
                        [](int a) { return a > 0; });
    #else
    const int non_stationary_farend = any_of(self->farend->far_bit_counts,self->history_size,ret_predicate);
    #endif
